...
![](assets/corr_pvalue_mito_percent.png){width="100%"}
...
#### Look into impact of cross-mappability to Dandelion 

First annotate the GWAS overlapped sceptre result with symmetric cross mappability (gene A and B can be reversed) score for each gene pair. 

It turns out that many pairs have cross mappability score of NA (Only 10-15% have the score). Then, further investigate this situation. 

By looking back to the paper, there are two possible reasons:

1. No cross mappability for these pairs
2. Gene mappability is 1 or close to 1

2 is more likely to be the case (since almost half of genes have mappability of 1). So, to validate this, I annotate the gene mappability of each gene:

1. at least one of the genes have gene mappability = 1, no shared k-mers
2. both gene mappability > 0.5, each k-mers only map to 1-2 position
3. only a few that gene mappability > 0.1

Especially for pairs detected by Dandelion, most of them have at least one gene that mappability is 1. 

What if we remove cross mappable pairs before running Dandelion analysis
  - The core genes are the same, but the regulators are less. 
...
1. scRNA - Basu
    1. scRNA: 36601 genes x 379736 cells
    2. 85 unique samples (46 patients): 
        1. 33 controls, 16 nons, 19 adjacent, 17 inf
        2. 51 females
        3. 37 ascending colon
        4. min age 19, max age 75, mean/median 45/48
    3. 3 unique major cell types: Epithelial, Immune, Mesenchymal
2. scATAC - Pott
    1. scATAC: 557310 peak x 178030 cells 
    2. 71 unique samples (31 patients):
        1. 29 controls, 42 CD
        2. 31 AC, 40 TI
    3. 3 unique cell types: epithelial, immune, stromal
3. scRNA - Pott
    1. scRNA: 23849 genes x 213275 cells
    2. 46 patients


#### Potential analysis 

1. Map Genes to Regulatory Regions
  1. Promoter Mapping
    - What it is: Looking for peaks that fall within the promoter region of a gene.
    
    - Biological rationale: Promoters are DNA sequences immediately upstream of transcription start sites (TSS) where RNA polymerase and transcription factors bind to initiate transcription. Peaks in promoter regions are direct regulators of gene expression.
    
    - Technical implementation:
    
      - TSS identification: For each gene, we identify the Transcription Start Site:
      
          - For + strand genes: TSS = gene start (left in your annotation)
      
          - For - strand genes: TSS = gene end (right in your annotation)
      
      - Promoter definition: We define promoter as TSS ± 2kb (default)
      
          - Upstream 2kb + Downstream 2kb = 4kb total region
  
      - Mapping: Any ATAC-seq peak that overlaps this promoter region is mapped to that gene
    
  2. Gene Body Mapping
  
    - What it is: Looking for peaks that fall within the transcribed region of a gene (exons + introns).
    
    - Biological rationale: Chromatin accessibility within gene bodies can influence:
    
      - Transcription elongation (RNA polymerase pausing/release)
    
      - Alternative splicing (by regulating spliceosome access)
    
      - Transcription termination
    
      - Epigenetic memory of recently transcribed genes
    
    - Technical implementation:
    
      - Gene body definition: The entire region from gene start to end (left to right in your annotation)
    
      - Mapping: Any ATAC-seq peak that overlaps this gene body region is mapped to that gene
      
      
  3. Proximity Mapping
    - What it is: Looking for peaks that are near but not overlapping a gene, up to a certain distance.
    
    - Biological rationale: Enhancers and other distal regulatory elements can be located far from genes (even megabases away) but regulate them through 3D chromatin looping. This method captures:

      - Distal enhancers that loop to contact promoters
      
      - Silencer elements that repress gene expression
      
      - Boundary elements that insulate genes

    - Technical implementation:

      - ind nearest gene: For each peak, find the closest gene's TSS
      
      - Distance calculation: Calculate genomic distance from peak to TSS
      
      - Distance cutoff: Keep only peaks within a threshold (default: 500kb)
      
      - No overlap requirement: Peaks don't need to overlap gene regions
      
2. Analyze Cell-Type Specific Accessibility
    1. Heritability enrichment analysis
    2. Identify Which Cell Types Show Accessibility at Gene Regions
3. Differential Accessibility Analysis
    1. Find Peaks that are Differentially Accessible Between Conditions

DE analysis

1. DE for different cell types
2. pvlaue limit 
3. extreme genes concerns
4. 1 overlapping of DPGs with DE: LCP2 in Macrophage M1 significant, other cell types not

Heritability enrichment analysis 

1. by using the ATAC seq data, by cell type
